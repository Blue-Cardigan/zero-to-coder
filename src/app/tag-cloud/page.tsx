...
export default function TagCloudPage() {
  ...
  // Color scheme for the tags - vibrant colors matching the slides
  ...
  const processStoredProcedureData = (data: TagResponse[]) => {
    try {
      ...
      const words: WordCloudWord[] = Object.keys(tagFrequency)
        ...
        .map(tag => {
          ...
          return {
            text: tag,
            size,
            color: colorScale(Math.floor(Math.random() * 8).toString())
          };
        })
  ...
  const fetchData = async (isInitialLoad = false) => {
    try {
      ...
      if (procError) {
        ...
        processStoredProcedureData(procData);
        ...
        function draw(words: any[]) {
          ...
          g.selectAll(".word")
            ...
            .style("fill", (d: any, i: number) => {
              ...
              const index = (parseInt((d as any).text.length.toString()) + i) % 8;
